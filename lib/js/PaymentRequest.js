...
import {
  MODULE_SCOPING,
  BILLING_CONTACT_CHANGE_EVENT,
  INTERNAL_BILLING_CONTACT_CHANGE_EVENT,
...
export default class PaymentRequest {
  ...
  _billingContact: null | PaymentAddress;
  ...
  _billingContactChangeSubscription: any;
  ...
  constructor(
    ...
    // 21. Set billing contact to null.
    this._billingContact = null;
  ...
  _setupEventListeners() {
    ...
    if (IS_IOS) {
      ...
      this._billingContactChangeSubscription = DeviceEventEmitter.addListener(
        INTERNAL_BILLING_CONTACT_CHANGE_EVENT,
        this._handleBillingContactChange.bind(this)
      );
    }
  }

  _handleBillingContactChange(postalAddress: PaymentAddress) {
    this._billingContact = postalAddress;
  }
  ...
  _handleUserAccept(details: {
    ...
    const paymentResponse = new PaymentResponse({
      ...
      payerEmail: this._options.requestPayerEmail
        ? details.payerEmail || this._shippingAddress.email
        : null,
      billingContact: this._options.requestBilling
  ...
  _removeEventListeners() {
    ...
    if (IS_IOS) {
      ...
      DeviceEventEmitter.removeSubscription(
        this._billingContactChangeSubscription
      );
    }
  }
