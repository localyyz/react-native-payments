...
export default class PaymentRequestUpdateEvent {
  ...
  constructor(name, target) {
    if (
      ...
      name !== BILLING_CONTACT_CHANGE_EVENT &&
      name !== SHIPPING_OPTION_CHANGE_EVENT
    ) {
      throw new Error(
        `Only "${SHIPPING_ADDRESS_CHANGE_EVENT}" and "${SHIPPING_OPTION_CHANGE_EVENT}" and "${BILLING_CONTACT_CHANGE_EVENT}" event listeners are supported.`
      );
    }
    ...
    return (
      NativePayments.handleDetailsUpdate(normalizedDetails, DOMException)
        ...
        .then(() => {
          ...
          // TODO: detech if shippingOptions has changed or not.
          // if user emits an SHIPPING_OPTION_CHANGE_EVENT, this will be
          // incorrectly reset to the default first option
          if (
            this.name === SHIPPING_ADDRESS_CHANGE_EVENT &&
